"""Medit .meshb binary codec (GmFlib format) — read + write with lazy support."""
...
def read(path: Path | str, *, lazy: bool = False) -> PolyData:
    ...
    lazy
        If True, vertex data is mmap-backed (zero-copy for float64 / v2 files).
        Version 1 (float32) files always copy vertices to float64 regardless.
    ...
    path = Path(path)
    if lazy:
        return _read_lazy(path)
    return _read_eager(path)


def write(poly: PolyData, path: Path | str, **opts: Any) -> None:
    """Serialise PolyData to a Medit binary mesh file (.meshb).

    Writes version 2 (float64, 32-bit counts). Reference integers default to 0.
    ...
    with open(path, "wb") as fh:
        ...
        for kw, indices in groups.items():
            ...
            for ei in indices:
                ...
                nodes = (poly.connectivity[s:e] + 1).astype(np.int32)
                fh.write(nodes.tobytes())
                fh.write(struct.pack("<i", 0))  # ref = 0
...
def _scan_sections(
    ...
    # Record sizes (bytes per element, including trailing ref int32)
    ...
    while pos + 8 <= total:
        ...
        if rec == 0:
            # Unknown keyword — cannot determine data size, stop scanning.
            break
...
def _read_eager(path: Path) -> PolyData:
    with open(path, "rb") as fh:
        mm = mmap.mmap(fh.fileno(), 0, access=mmap.ACCESS_READ)
        try:
            return _decode(mm, lazy=False)
        finally:
            mm.close()


def _read_lazy(path: Path) -> PolyData:
    with open(path, "rb") as fh:
        mm = mmap.mmap(fh.fileno(), 0, access=mmap.ACCESS_READ)
        try:
            return _decode(mm, lazy=True)
        finally:
            mm.close()


def _decode(mm: mmap.mmap, *, lazy: bool) -> PolyData:
    ...
    if _KW_VERTICES in sections:
        ...
        if lazy and version == 2:
            verts_arr = np.frombuffer(
                memoryview(mm)[vstart : vstart + nbytes], dtype=vert_dt
            )
        else:
            verts_arr = np.frombuffer(
                bytes(mm[vstart : vstart + nbytes]), dtype=vert_dt
            )
